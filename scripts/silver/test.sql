...
SELECT
    ...
    ci.cst_marital_status,
      CASE WHEN ci.cst_gender != 'Unknown' THEN ci.cst_gender
        ELSE COALESCE(ca.gen, 'Unknown') END AS new_gen,
    ci.cst_creationdate,
    ca.bdate,
    la.cntry
...
SELECT
    ...
    ci.cst_creationdate AS creation_date   
FROM silver.crm_cust_info AS ci
LEFT JOIN silver.erp_cust_az12 AS ca
    ON ci.cst_key = ca.cid
LEFT JOIN silver.erp_loc_a101 AS la
...
SELECT DISTINCT
    cst_gender FROM silver.crm_cust_info
...
ON pn.cat_id = pc.id
WHERE pn.prd_end_dt IS NULL --Filter out all hostorical data
)t GROUP BY prd_key
HAVING COUNT(*) >1
